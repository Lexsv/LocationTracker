...
class RegisterPresent @Inject constructor(val registerView: RegisterView) :
    ...
    override fun registNew(name: String, email: String, password: String, repitPassword: String) {
        if (isNet(ProvidContext.getContext())) {
            val resalt = validEnterDataRegister(
                                                        name = name,
                                                        email = email,
                                                        password = password,
                ...
                mAuth.createUserWithEmailAndPassword(email, password)
                    .addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            registerView.getVM().creatUserInfo(mAuth)
                            ...
                            mAuth.currentUser!!.updateProfile(
                                ...
                                .addOnCompleteListener { taskUpData ->
                                    if (taskUpData.isSuccessful) {
                                        registerView.getVM().upDataRegistr(mAuth)
                            ...
                            if (task.getException() is FirebaseAuthException) {
                                ...
                                when ((task.getException() as FirebaseAuthException).errorCode) {
                                    "ERROR_WRONG_PASSWORD" -> registerView.actionMassege(
                                        ActionMessage.ERROR_WRONG_PASSWORD.result
                                    )
                                    "ERROR_USER_NOT_FOUND" -> registerView.actionMassege(
                                        ActionMessage.ERROR_USER_NOT_FOUND.result
                                    )
                                    "ERROR_EMAIL_ALREADY_IN_USE" -> registerView.actionMassege(
                                        ActionMessage.ERROR_EMAIL_ALREADY_IN_USE.result
                                    )
                                }
                            }
                        }
